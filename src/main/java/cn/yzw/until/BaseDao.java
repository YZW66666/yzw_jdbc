...
public class BaseDao {
    ...
    public boolean  getConncetion(){
        try {
            //加载驱动
            Class.forName(ConfigManage.getValue("jdbc.driver"));
            connection= DriverManager.getConnection(ConfigManage.getValue("jdbc.url"),
                    ConfigManage.getValue("jdbc.username"),
                    ConfigManage.getValue("jdbc.password"));
        } catch (Exception e) {
            e.printStackTrace();
            return  false;
        }
